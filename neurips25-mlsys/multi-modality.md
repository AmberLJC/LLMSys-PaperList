...
#### VisionThink: Smart and Efficient VLM Training via Reinforcement Learning

**Link:** https://github.com/dvlab-research/VisionThink

**Authors:** Senqiao Yang, Junyi Li, Xin Lai, Bei Yu, Hengshuang Zhao, Jiaya Jia

**Summary:** VisionThink uses reinforcement learning to autonomously learn whether to reduce visual tokens during training. Unlike traditional efficient VLM approaches using fixed compression, VisionThink dynamically decides compression levels, achieving significant improvements on fine-grained benchmarks involving OCR-related tasks while reducing computational overhead during training.
...
**Summary:** MeanFlow presents a principled one-step generative framework using average velocity rather than instantaneous velocity. Requiring no pre-training, distillation, or curriculum learning, MeanFlow achieves FID **3.43** on ImageNet 256×256 with a single function evaluation, significantly outperforming previous one-step models and narrowing the gap with multi-step methods.
...
**Summary:** First successful demonstration of normalizing flows at large scale and high resolution. STARFlow combines normalizing flows with autoregressive transformers, generating high-resolution images without the computational cost of diffusion models or quantization artifacts of AR models. The approach maintains exact likelihood modeling with faster training and inference than comparable methods.
...
**Authors:** Jie Liu, Gongye Liu, Jiajun Liang, Ziyang Yuan, et al.

**Summary:** Flow-GRPO-Fast requires training on only **one or two denoising steps** per trajectory. Generates deterministic trajectories using ODE sampling, injects noise at random intermediate steps, then continues with ODE. Confining stochasticity to minimal steps significantly reduces training cost while matching reward performance.

---

#### Align Your Flow: Flow Map Models for Few-step Generation

**Link:** https://vectorinstitute.ai/

**Authors:** Vector Institute Researchers

**Summary:** Develops flow map methods achieving state-of-the-art few-step generation that generalizes flow, diffusion, and consistency models. Demonstrates autoguidance can improve performance using low-quality models during distillation. Achieves SOTA few-step generation on ImageNet 64×64 and 512×512 using small, efficient neural networks.
